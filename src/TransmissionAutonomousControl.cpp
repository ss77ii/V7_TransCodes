#include "main.h"
#include "TransmissionConfiguration.hpp"

#define PRE 2
#define CATCH 1
#define RELEASE 0

#define ON 1
#define OFF -1

// go straight parameters

//	goStraightCmPID_lib(122, 90, 127, MOVE_FORWARD, 5, 0, 3, 0.35, 0, 26, 6000, 3, hardwareParameter);
//	goStraightCmPID_lib(122, 90, 127, MOVE_FORWARD, 5, 0, 3, 0.35, 0, 24, 6000, 2, hardwareParameter);
//	goStraightCmPID_lib(122, 90, 127, MOVE_FORWARD, 5, 0, 3, 0.3, 0, 28, 6000, 1, hardwareParameter);

// goStraightCmPID_lib(60.0, 90, 127, MOVE_BACKWARD, 5, 0, 3, 0.40, 0, 1, 2000, 2, hardwareParameter);

// left is positive, right is negative

using namespace pros;

Point curPos;
double distance;
double angle;

/***********************************************************************************************

AUTONOMOUS PROGRAMS

***********************************************************************************************/

/************************************************************************
// ENODEV; 19 - The port cannot be configured as a vision sensor
// EINVAL; 22 - sig_id is outside the range [1-7]
// EDOM;   33 - size_id is greater than the number of available objects.
// EAGAIN; 11 - Reading the vision sensor failed for an unknown reason.
**************************************************************************/

bool robot_staled = false;
bool check = false;

void transmission_con_fn(void *param)
{
	while (true)
	{
		if (robot_staled)
		{
			if (!check)
			{
				std::cout << "Lmao" << std::endl;
				int degrees = Y_encoder.get_value();
				delay(1000);
				int second_degree = Y_encoder.get_value();
				std::cout << degrees << " - " << second_degree << std::endl;

				if (abs(second_degree - degrees) <= 70)
				{
					std::cout << "lol" << std::endl;
					transmission_piston.set_value(true);
					armAction_1 = {127, 0, -20, 1};
					check = true;
				}
			}
		}
		else
		{
			transmission_piston.set_value(false);
		}
	}
}

pros::Task transmission_con(transmission_con_fn, (void *)"PROS", TASK_PRIORITY_DEFAULT, TASK_STACK_DEPTH_DEFAULT, "transmission condition task");

detected_vision_goal_lib get_goal_object_front_vision(int goal_color_signature)
{
	detected_vision_goal_lib final_goal = {goal_color_signature, 0, 0, 0, 0, 0, 0};
	int e_number1;
	int e_number2;
	vision_object_s_t left_goal;
	vision_object_s_t right_goal;

	errno = 6;
	vision_object_s_t largest_goal = front_vision.get_by_sig(0, goal_color_signature);
	std::cout << "   mid = " << largest_goal.x_middle_coord << "...";
	e_number1 = errno;
	errno = 6;
	vision_object_s_t second_goal = front_vision.get_by_sig(1, goal_color_signature);
	e_number2 = errno;

	if (e_number1 == 33)
	{
		final_goal.width = -1;
		final_goal.height = -1;
		std::cout << "   111   ";
		return final_goal;
	}

	final_goal.left_coord = largest_goal.left_coord;
	final_goal.top_coord = largest_goal.top_coord;
	final_goal.width = largest_goal.width;
	final_goal.height = largest_goal.height;
	final_goal.x_middle_coord = largest_goal.x_middle_coord;
	final_goal.y_middle_coord = largest_goal.y_middle_coord;
	if (e_number2 == 33)
	{ // only one object detected
		std::cout << "   222   ";
		return final_goal;
	}
	else
	{ // two object detected
		if (largest_goal.width < 40)
		{ // object too small to combine
			std::cout << "   333   ";
			return final_goal;
		}
		else
		{ // object big/close enough to combine
			if ((abs(largest_goal.top_coord - second_goal.top_coord) <= 5) && (abs(largest_goal.height - second_goal.height) <= 8))
			{ // two goals with similar height
				if (largest_goal.left_coord < second_goal.left_coord)
				{
					left_goal = largest_goal;
					right_goal = second_goal;
				}
				else
				{
					left_goal = second_goal;
					right_goal = largest_goal;
				}
				if (left_goal.left_coord + left_goal.width < right_goal.left_coord)
				{ // two goals not overlapped
					final_goal.left_coord = left_goal.left_coord;
					final_goal.top_coord = left_goal.top_coord;
					final_goal.width = right_goal.left_coord - left_goal.left_coord + right_goal.width;
					final_goal.height = left_goal.height;
					final_goal.x_middle_coord = final_goal.left_coord + final_goal.width / 2;
					final_goal.y_middle_coord = final_goal.top_coord + final_goal.height / 2;
					std::cout << "   444   ";
					return final_goal;
				}
				else
				{ // two goals overlapped
					std::cout << "   555   ";
					return final_goal;
				}
			}
			else
			{ // two goals are in different height
				std::cout << "   666   ";

				return final_goal;
			}
		}
	}

	std::cout << "   777   ";
	return final_goal;
}

void arm_moving_holding_fn(void *param)
{
	long start_time = pros::millis();

	arm_action_lib = ARM_HOLDING_POSITION;
	arm_motor.move(0);
	delay(200);
	while (true)
	{
		if (fabs(arm_motor.get_actual_velocity()) < 3)
		{
			break;
		}
		if (pros::millis() - start_time > 500)
		{
			break;
		}
		delay(10);
	}
	arm_motor.tare_position();
	arm_motor.move_absolute(10, 127);
	delay(50);
	arm_motor.tare_position();

	int pass_target_count = 0;
	double pre_arm_target_angle = arm_move_target_angle_lib;
	double cur_arm_degree = arm_motor.get_position();
	double pre_arm_degree = cur_arm_degree;
	double Kp = 3, Ki = 0.005, Kd = 0;
	double error_kp = 0;
	double pre_error_kp = 0;
	double error_sum = 0;
	double error_change_rate = 0;
	double total_correction = 0;

	while (true)
	{
		if (pre_arm_target_angle != arm_move_target_angle_lib)
		{
			pre_arm_target_angle = arm_move_target_angle_lib;
			error_sum = 0;
			error_kp = arm_move_target_angle_lib - cur_arm_degree;
			pre_error_kp = error_kp;
			pass_target_count = 0;
		}
		cur_arm_degree = arm_motor.get_position();
		if (pass_target_count == 0)
		{
			if (cur_arm_degree == arm_move_target_angle_lib || (cur_arm_degree > arm_move_target_angle_lib && pre_arm_degree < arm_move_target_angle_lib) || (cur_arm_degree < arm_move_target_angle_lib && pre_arm_degree > arm_move_target_angle_lib))
			{
				pass_target_count++;
				//				pros::lcd::print(1, "AAAA=%.1f", arm_move_target_angle_lib);
			}
			else
			{
				if (cur_arm_degree < arm_move_target_angle_lib)
				{
					arm_motor.move(abs(arm_move_speed_lib));
				}
				else if (cur_arm_degree > arm_move_target_angle_lib)
				{
					arm_motor.move(0 - abs(arm_move_speed_lib));
				}
			}
		}
		else
		{
			error_kp = arm_move_target_angle_lib - cur_arm_degree;
			error_sum = error_sum + error_kp;
			error_change_rate = error_kp - pre_error_kp;
			total_correction = error_kp * Kp + error_sum * Ki + error_change_rate * Kd;
			arm_motor.move((int)total_correction);
			pre_error_kp = error_kp;
		}
		pre_arm_degree = cur_arm_degree;
		//		pros::lcd::print(3, "arm=%.1f", arm_motor.get_position());
		delay(5);
	}
}

void set_all_motor_power(int climb_speed)
{
	left_front_motor.move(climb_speed);
	left_mid_motor.move(climb_speed);
	left_back_motor.move(climb_speed);
	right_front_motor.move(climb_speed);
	right_mid_motor.move(climb_speed);
	right_back_motor.move(climb_speed);
}

void auton_60s_skills_bridge_version()
{
	sys_initial_to_auton_drifting = inertial_sensor.get_rotation();
	sys_initial_robot_heading = 180;
	long hook_action_delay_time = 50;
	long claw_action_delay_time = 120;
	long start_time = pros::millis();
	int intake_speed = 107;
	arm_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	double roll;
	clawAction_1 = {0, false, 1};

	//	pros::Task arm_holding_task(arm_moving_holding_fn, (void*)"PROS", TASK_PRIORITY_DEFAULT, TASK_STACK_DEPTH_DEFAULT, "arm_holding_task");

	// sys_display_info_terminal = DEBUG_DISPLAY_MASSAGE_ON;
	// top_piston.set_value(false);
	// sys_display_info_terminal = DEBUG_DISPLAY_MASSAGE_ON;
	// goStraightCm_Front_Vision(55, 90, 100, DETECT_YELLOW_GOAL_SIG, front_vision,
	// 			              						0.5, 0, 1, 0.3, 0, 10, 0.8, 0, 0, 1000, 1, hardwareParameter);
	// front_piston.set_value(true);
	// pros::lcd::print(3, "bbbbbbbbbbbbbbbbb");
	// waitForTouch();
	// pros::lcd::print(3, "aaaaaaaaaaaaaaaaa");
	// while(true){
	// 	delay(50);
	// }

	// int num1 = 0;
	// int num2 = 0;
	// while(true){
	// 	if(is_limit_switch_pressed() == true){
	// 		num1++;
	// 		pros::lcd::print(1, "pressed = %d", num1);
	// 	} else {
	// 	  num2++;
	// 		pros::lcd::print(2, "not pressed%d", num2);
	//   }
	// 	delay(10);
	// }

	// //	bridge code
	//    sys_initial_robot_heading = 135;
	//
	// clawAction_1 = {0, true, 1};
	// hookAction_1 = {0, true, 1};
	// delay(100);
	// turnDegreesPID_lib(180, ON_SPOT_TURN, 127, COUNTER_CLOCKWISE, 6, 0, -20, 1000, 2, hardwareParameter);
	// waitForTouch();
	// 	clawAction_1 = {0, true, 1};
	// 	delay(claw_action_delay_time);
	// 	armAction_1 = {127, 0, PRESS_BRIDGE + 100, 1};
	// 	delay(300);
	// 	goStraightCmPID_lib(20, 185, 127, MOVE_FORWARD, 8, 0, 0, 0.2, 0, 5, 800, 1, hardwareParameter);
	// 	delay(100);
	// 	goStraightCmPID_lib(35, 183, 127, MOVE_FORWARD, 1.2, 0, 0, 0.5, 0, 5, 1200, 1, hardwareParameter);
	// 	delay(100);
	// //	turnDegreesPID_lib(185, ON_SPOT_TURN, 70, COUNTER_CLOCKWISE, 6, 0, -20, 900, 2, hardwareParameter);
	// //	goStraightCmPID_lib(30, 185, 127, MOVE_FORWARD, 3, 0, 0, 1, 0, 5, 1000, 1, hardwareParameter);
	// //waitForTouch();
	// 	armAction_1 = {127, 0, 20, 1};
	// 	delay(800);
	// 	left_front_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// 	left_back_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// 	left_mid_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// 	right_front_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// 	right_back_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// 	right_mid_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	//
	// 	goStraightCmPID_lib(85, 180, 127, MOVE_FORWARD, 3, 0, 0, 10, 0, 0, 2000, 1, hardwareParameter);
	// 	balance_bridge_PID_lib(127, -15, 15, 0, 0, 5000, 1, hardwareParameter);
	// 	goStraightCmPID_lib(3.5, 180, 127, MOVE_BACKWARD, 0, 0, 0, 1, 0, 0, 800, 1, hardwareParameter);
	//
	// 	pros::lcd::print(2, "Time=%d", pros::millis() - start_time);
	// 	waitForTouch();

	// 	hookAction_1 = {0, true, 1};
	// 	clawAction_1 = {0, true, 1};
	// 	delay(claw_action_delay_time);
	// 		left_front_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// 		left_back_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// 		left_mid_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// 		right_front_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// 		right_back_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD) ;
	// 		right_mid_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);

	// 		goStraightCmPID_lib(85, 180, 127, MOVE_FORWARD, 3, 0, 0, 10, 0, 0, 3000, 1, hardwareParameter);
	// 		balance_bridge_PID_lib(127, -18, 15, 0, 0, 5000, 1, hardwareParameter);
	// 		goStraightCmPID_lib(1.5, 180, 127, MOVE_BACKWARD, 0, 0, 0, 3, 0, 0, 800, 1, hardwareParameter);

	// pros::lcd::print(2, "Time=%d", pros::millis() - start_time);
	// std::cout << "Time=%d" << pros::millis() - start_time << std::endl;
	// waitForTouch();

	//	hookAction_1 = {0, false, 1};
	//	transmission_piston.set_value(false);
	//	top_piston.set_value(false);

	// 	clawAction_1 = {0, true, 1};
	// 	hookAction_1 = {0, true, 1};
	// 	armAction_1 = {127, 0, PRESS_BRIDGE + 80, 1};
	// 	delay(1500);
	// 	goStraightCmPID_lib(10, 180, 127, MOVE_FORWARD, 2, 0, 0, 10, 0, 0, 800, 1, hardwareParameter);
	// 	transmission_piston.set_value(true);
	// 	armAction_1 = {127, 0, 40, 1};
	// 	delay(1500);
	// //	waitForTouch();
	// 	goStraightCmPID_lib(100, 180, 127, MOVE_FORWARD, 1, 0, 0, 10, 0, 0, 3000, 1, hardwareParameter);
	//  	balance_bridge_PID_lib(127, -18, 15, 0, 0, 5000, 1, hardwareParameter);
	//  	goStraightCmPID_lib(2.5, 180, 127, MOVE_BACKWARD, 0, 0, 0, 1, 0, 0, 800, 1, hardwareParameter);

	// 	waitForTouch();
	// intake_motor.set_brake_mode(E_MOTOR_BRAKE_BRAKE);
	// left_front_motor.set_brake_mode(E_MOTOR_BRAKE_COAST);
	// left_back_motor.set_brake_mode(E_MOTOR_BRAKE_COAST);
	// left_mid_motor.set_brake_mode(E_MOTOR_BRAKE_COAST);
	// right_front_motor.set_brake_mode(E_MOTOR_BRAKE_COAST);
	// right_back_motor.set_brake_mode(E_MOTOR_BRAKE_COAST);
	// right_mid_motor.set_brake_mode(E_MOTOR_BRAKE_COAST);

	// 	goStraightCmPID_lib(50, 180, 100, MOVE_FORWARD, 0, 0, 0, 1, 0, 10, 1500, 1, hardwareParameter);
	// 	double distance1 = Y_encoder.get_value()*ENCODER_CIRCUMFERENCE_CM/360;
	// 	pros::lcd::print(5, "cm: %.1f, deg: %d", distance, Y_encoder.get_value());

	// waitForTouch();

	// while(true){
	// 	distance = Y_encoder.get_value()*ENCODER_CIRCUMFERENCE_CM/360;
	// 	pros::lcd::print(5, "cm: %.1f, deg: %d", distance, Y_encoder.get_value());
	// 	delay(30);
	// }

	// waitForTouch();

	// 	clawAction_1 = {0, true, 1};
	// 	hookAction_1 = {0, true, 1};
	// 	armAction_1 = {127, 0, 25, 1};
	// 	delay(300);
	// 	transmission_piston.set_value(true);
	// 	goStraightCmPID_lib(70, 180, 127, MOVE_FORWARD, 3, 0, 0, 10, 0, 0, 3000, 1, hardwareParameter);

	//    set_all_motor_power(60);
	// 	while (true){
	// 		roll = inertial_sensor.get_roll();
	// 		if (roll > -18){
	// 			break;
	// 		}
	// 		delay(10);
	// 	}
	//    set_all_motor_power(-50);
	// 	delay(200);
	//    set_all_motor_power(0);
	//  waitForTouch();

	// ------------------------------------------------------------------------------------ //
	//// part 1 take first blue goal
	armAction_1 = {127, 201, 30, 1};
	goStraightCmPID_lib(10, 180, 78, MOVE_BACKWARD, 0, 0, 0, 5, 0, 0, 500, 1, hardwareParameter);
	hookAction_1 = {0, true, 1};
	delay(hook_action_delay_time);
	angle = 82;
	intakeAction_1 = {intake_speed, 200, 0, intake_speed, 1};
	goStraightCmPID_lib(65, angle, 127, MOVE_FORWARD, 2, 0, 2.5, 2, 0, 0, 1500, 1, hardwareParameter); // 100,80,127
	goStraightCm_Front_Vision_limit_switch(67, angle, 100, DETECT_YELLOW_GOAL_SIG, front_vision,
										   0.4, 0, 1.5, 0.3, 0, 10, 0.4, 0, 0, 1200, 1, hardwareParameter);

	///////catch first yellow goal
	//	delay(50);
	clawAction_1 = {80, true, 1};
	delay(150);
	//	armAction_1 = {127, 200, RELEASE_BRIDGE + 100, 1};
	armAction_1 = {127, 200, RELEASE_BRIDGE, 1};

	///////carry first yellow goal to bridge
	angle = 38;
	goStraightCmPID_lib(154, angle, 110, MOVE_FORWARD, 3.5, 0, 2.5, 0.3, 0, 5, 1800, 1, hardwareParameter); // 1650
	angle = 115;
	turnDegreesPID_lib(angle, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 25, 1000, 2, hardwareParameter);
	goStraightCmPID_lib(40, angle, 127, MOVE_FORWARD, 2.5, 0, 2.5, 0.4, 0, 5, 700, 1, hardwareParameter); // 1650
	///////place the first yellow goal to the bridge
	armAction_1 = {127, 0, PRESS_BRIDGE - 20, 1};
	delay(300);
	clawAction_1 = {0, false, 1};
	delay(100);
	intakeAction_1 = {0, 500, 0, 0, 1};
	// drop first blue
	hookAction_1 = {0, false, 1};
	goStraightCmPID_lib(45, angle, 127, MOVE_BACKWARD, 5, 0, 2.5, 0.5, 0, 4, 1000, 1, hardwareParameter);
	armAction_1 = {127, 0, 0, 1};
	delay(100);
	goStraightCmPID_lib(22, angle, 100, MOVE_FORWARD, 5, 0, 1, 0.5, 0, 10, 700, 1, hardwareParameter);
	turnDegreesPID_lib(285, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 25, 1100, 2, hardwareParameter);
	// pick up first blue
	goStraightCmPID_lib_limit_switch(30, angle + 180, 80, MOVE_FORWARD, 5, 0, 1, 0.5, 0, 10, 800, 1, hardwareParameter);
	clawAction_1 = {0, true, 1};
	armAction_1 = {127, 0, PRESS_BRIDGE + 60, 1};
	delay(150);

	// place first blue
	angle = 93;
	turnDegreesPID_lib(angle, ON_SPOT_TURN, 70, COUNTER_CLOCKWISE, 6, 0, 20, 1100, 2, hardwareParameter);
	goStraightCmPID_lib(69, angle, 127, MOVE_FORWARD, 5, 0, 1, 0.5, 0, 10, 1300, 1, hardwareParameter);
	armAction_1 = {127, 0, PRESS_BRIDGE + 65, 1};
	delay(50);
	clawAction_1 = {0, false, 1};
	delay(100);
	goStraightCmPID_lib_limit_switch(41, angle, 127, MOVE_BACKWARD, 5, 0, 1, 0.5, 0, 10, 800, 1, hardwareParameter);

	// get middle yellow
	armAction_1 = {127, 0, 0, 1};
	turnDegreesPID_lib(250, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 25, 1100, 2, hardwareParameter);
	goStraightCm_Front_Vision_limit_switch(60, 250, 100, DETECT_YELLOW_GOAL_SIG, front_vision,
										   0.4, 0, 1.5, 0.3, 0, 10, 0.4, 0, 0, 1200, 1, hardwareParameter);
	clawAction_1 = {0, true, 1};
	delay(200);
	armAction_1 = {127, 0, 50, 1};

	// int direction = COUNTER_CLOCKWISE;
	// double angle1 = get_robot_heading_lib(hardwareParameter);
	// if (angle1 > 300){
	// 	direction = CLOCKWISE;
	// }
	// turnDegreesPID_lib(300, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 25, 700, 2, hardwareParameter);
	// goStraightCmPID_lib(100, 300, 127, MOVE_BACKWARD, 5, 0, 1, 0.5, 0, 10, 1500, 1, hardwareParameter);
	goStraightCmPID_lib(104, 300, 127, MOVE_BACKWARD, 6, 0, 1, 0.5, 0, 10, 1500, 1, hardwareParameter);
	turnDegreesPID_lib(355, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 25, 1100, 1, hardwareParameter);
	double distance = get_distance_back_vision(back_vision, DETECT_RED_GOAL_SIG, 10, 10, 60, 120);

	// get corner red
	goStraightCm_Back_Vision(distance + 10, 340, 80, DETECT_RED_GOAL_SIG, back_vision,
							 0.5, 0, 1, 0.4, 0, 5, 0.3, 0, 5, 1300, 1, hardwareParameter);
	delay(100);
	hookAction_1 = {0, true, 1};
	delay(150);
	armAction_1 = {127, 200, PRESS_BRIDGE + 100, 1};
	// place middle yellow
	turnDegreesPID_lib(60, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 25, 1100, 2, hardwareParameter);
	goStraightCmPID_lib(43, 60, 127, MOVE_FORWARD, 5, 0, 1, 0.5, 0, 10, 1000, 1, hardwareParameter);
	armAction_1 = {127, 400, PRESS_BRIDGE + 30, 1};
	goStraightCmPID_lib(43, 5, 127, MOVE_FORWARD, 6, 0, 1, 1, 0, 10, 1200, 1, hardwareParameter);
	armAction_1 = {127, 200, PRESS_BRIDGE, 1};
	delay(200);
	clawAction_1 = {0, false, 1};
	delay(100);

	// back away
	goStraightCmPID_lib(55, 25, 127, MOVE_BACKWARD, 6, 0, 1, 1, 0, 10, 1200, 1, hardwareParameter);
	turnDegreesPID_lib(315, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 27, 800, 1, hardwareParameter);
	intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	lcd::print(5, "Angle: %f", get_robot_heading_lib(hardwareParameter));

	//	675, 504, 510
	// go across the field
	goStraightCmPID_lib(240, 315, 127, MOVE_FORWARD, 5, 0, 1, 1, 0, 10, 2500, 1, hardwareParameter);
	armAction_1 = {127, 0, RELEASE_BRIDGE, 1};

	turnDegreesPID_lib(125, ON_SPOT_TURN, 120, CLOCKWISE, 6, 0, 25, 1100, 2, hardwareParameter);
	hookAction_1 = {0, false, 1};
	goStraightCmPID_lib(70, 135, 127, MOVE_BACKWARD, 2.5, 0, 1, 2, 0, 10, 1400, 1, hardwareParameter);
	//  vision_object_s_t blue_goal = front_vision.get_by_sig(0, DETECT_BLUE_GOAL_SIG);
	// 		first_goal = vision1.get_by_sig(0, DETECT_RED_GOAL_SIG);

	// pros::lcd::print(4, "angle = %d", blue_goal.angle);
	//  pros::lcd::print(3, "Time=%d, %.1f", pros::millis() - start_time, get_robot_heading_lib(hardwareParameter));
	//  waitForTouch();

	// drop back goal in corner
	delay(100);
	goStraightCmPID_lib(34, 135, 127, MOVE_FORWARD, 6, 0, 1, 1, 0, 10, 1000, 1, hardwareParameter);
	turnDegreesPID_lib(180, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 25, 900, 2, hardwareParameter);
	distance = get_distance_back_vision(back_vision, DETECT_BLUE_GOAL_SIG, 10, 10, 60, 120);
	goStraightCm_Back_Vision(distance + 10, 340, 80, DETECT_BLUE_GOAL_SIG, back_vision,
							 0.5, 0, 1, 0.4, 0, 5, 0.3, 0, 5, 1200, 1, hardwareParameter);
	hookAction_1 = {0, true, 1};
	delay(50);
	armAction_1 = {127, 0, 0, 1};
	intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	turnDegreesPID_lib(120, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 25, 1100, 2, hardwareParameter);
	goStraightCm_Front_Vision_limit_switch(80, 250, 100, DETECT_YELLOW_GOAL_SIG, front_vision,
										   0.4, 0, 1.5, 0.3, 0, 10, 0.4, 0, 0, 1200, 1, hardwareParameter);
	clawAction_1 = {0, true, 1};
	delay(150);
	armAction_1 = {127, 0, PRESS_BRIDGE + 60, 1};

	// place third yellow
	goStraightCmPID_lib(135, 130, 127, MOVE_FORWARD, 6, 0, 1, 1, 0, 10, 2000, 1, hardwareParameter);
	clawAction_1 = {0, false, 1};
	delay(200);
	hookAction_1 = {0, false, 1};
	goStraightCmPID_lib(50, 125, 127, MOVE_BACKWARD, 5, 0, 2.5, 0.5, 0, 4, 1000, 1, hardwareParameter);
	armAction_1 = {127, 0, 0, 1};
	delay(100);
	goStraightCmPID_lib(27, 125, 100, MOVE_FORWARD, 5, 0, 1, 0.5, 0, 10, 700, 1, hardwareParameter);
	turnDegreesPID_lib(305, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 25, 1100, 2, hardwareParameter);
	// pick up blue
	//  goStraightCmPID_lib_limit_switch(30, 255, 80, MOVE_FORWARD, 5, 0, 1, 0.5, 0, 10, 800, 1, hardwareParameter);
	goStraightCm_Front_Vision_limit_switch(40, 305, 80, DETECT_BLUE_GOAL_SIG, front_vision,
										   0.4, 0, 1.5, 0.3, 0, 10, 0.5, 0, 0, 1200, 1, hardwareParameter);

	clawAction_1 = {0, true, 1};
	delay(200);
	armAction_1 = {127, 0, PRESS_BRIDGE + 60, 1};
	turnDegreesPID_lib(118, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 25, 1100, 2, hardwareParameter);
	goStraightCmPID_lib(75, 118, 127, MOVE_FORWARD, 6, 0, 1, 1, 0, 10, 1700, 1, hardwareParameter);
	clawAction_1 = {0, false, 1};
	delay(150);

	// place 2nd blue goal on bridge

	goStraightCmPID_lib(50, 102, 127, MOVE_BACKWARD, 5, 0, 2.5, 0.5, 0, 4, 1000, 1, hardwareParameter);
	intakeAction_1 = {0, 0, 0, 0, 1};
	turnDegreesPID_lib(40, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 25, 1100, 1, hardwareParameter);

	goStraightCmPID_lib(90, 35, 127, MOVE_FORWARD, 5, 0, 2.5, 0.5, 0, 4, 1000, 1, hardwareParameter);
	turnDegreesPID_lib(-15, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 25, 1100, 1, hardwareParameter);

	// get second red goal
	distance = get_distance_back_vision(back_vision, DETECT_RED_GOAL_SIG, 10, 10, 60, 120);
	goStraightCm_Back_Vision(distance + 8, 340, 80, DETECT_RED_GOAL_SIG, back_vision,
							 0.5, 0, 1, 0.4, 0, 5, 0.3, 0, 5, 1300, 1, hardwareParameter);
	delay(100);
	hookAction_1 = {0, true, 1};

	armAction_1 = {127, 0, 30, 1};
	intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	goStraightCmPID_lib(180, 275, 127, MOVE_FORWARD, 5, 0, 2.5, 0.5, 0, 4, 2000, 1, hardwareParameter);
	armAction_1 = {127, 0, 25, 1};
	goStraightCm_Front_Vision_limit_switch(130, 270, 90, DETECT_RED_GOAL_SIG, front_vision,
										   0.4, 0, 1.5, 0.3, 0, 10, 0.5, 0, 0, 1200, 1, hardwareParameter);
	clawAction_1 = {0, true, 1};
	delay(200);
	armAction_1 = {127, 0, PRESS_BRIDGE + 80, 1};
	goStraightCmPID_lib(13, 270, 127, MOVE_BACKWARD, 3, 0, 2.5, 0.5, 0, 4, 1000, 1, hardwareParameter);
	goStraightCmPID_lib(70, 270, 80, MOVE_FORWARD, 3, 0, 2.5, 0.5, 0, 4, 1200, 1, hardwareParameter);
	delay(100);
	goStraightCmPID_lib(5, 270, 90, MOVE_BACKWARD, 3, 0, 2.5, 0.5, 0, 4, 500, 1, hardwareParameter);
	turnDegreesPID_lib(180, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 25, 1100, 2, hardwareParameter);
	goStraightCmPID_lib(10, 180, 127, MOVE_FORWARD, 3, 0, 2.5, 0.5, 0, 4, 1000, 1, hardwareParameter);

	armAction_1 = {127, 0, 40, 1};
	delay(300);
	transmission_piston.set_value(true);
	goStraightCmPID_lib(70, 180, 127, MOVE_FORWARD, 3, 0, 0, 10, 0, 0, 3000, 1, hardwareParameter);

	set_all_motor_power(60);
	while (true)
	{
		roll = inertial_sensor.get_roll();
		if (roll > -18)
		{
			break;
		}
		delay(10);
	}
	set_all_motor_power(-50);
	delay(150);
	set_all_motor_power(0);

	waitForTouch();

	////take first blue goal by front claw
	// armAction_1 = {127, 0, 0, 1};
	// hookAction_1 = {0, false, 1};
	// delay(hook_action_delay_time);
	// goStraightCmPID_lib(27, 50, 100, MOVE_FORWARD, 5, 0, 1, 0.5, 0, 10, 700, 1, hardwareParameter);
	// turnDegreesPID_lib(230, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 25, 1100, 2, hardwareParameter);
	// goStraightCmPID_lib_limit_switch(30, 230, 80, MOVE_FORWARD, 5, 0, 1, 0.5, 0, 10, 800, 1, hardwareParameter);
	// clawAction_1 = {0, true, 1};
	// armAction_1 = {127, 0, 350, 1};
	// delay(150);

	// goStraightCmPID_lib(68, 0, 127, MOVE_BACKWARD, 1.2, 0, 1.2, 0.8, 0, 10, 1000, 1, hardwareParameter); // 1500
	// distance = get_distance_back_vision(back_vision, DETECT_RED_GOAL_SIG, 10, 10, 60, 120);
	// if (distance > 70 || distance < 10)
	// {
	// 	distance = 20;
	// }

	// goStraightCm_Back_Vision(distance + 20, 350, 65, DETECT_RED_GOAL_SIG, back_vision,
	// 						 0.5, 0, 1, 0.5, 0, 5, 0.5, 0, 5, 800, 1, hardwareParameter);

	// ////take first red goal by back claw
	// hookAction_1 = {0, true, 1};
	// armAction_1 = {127, 5, PRESS_BRIDGE + 300, 1};
	// intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	// ////catch first five flower ring
	// goStraightCmPID_lib(112, 340, 127, MOVE_FORWARD, 3.5, 0, 2.5, 0.3, 0, 5, 1500, 1, hardwareParameter);

	// goStraightCmPID_lib(23, 10, 100, MOVE_FORWARD, 2.5, 0, 0, 0.8, 0, 2, 800, 1, hardwareParameter); // 1300
	// goStraightCmPID_lib(25, 60, 100, MOVE_FORWARD, 2.5, 0, 0, 0.8, 0, 2, 800, 1, hardwareParameter); // 1300
	// armAction_1 = {127, 500, PRESS_BRIDGE, 1};
	// goStraightCmPID_lib(50, 90, 100, MOVE_FORWARD, 2.5, 0, 2, 1, 0, 5, 1100, 1, hardwareParameter);

	// delay(200);
	// clawAction_1 = {0, false, 1};
	// delay(50);
	// goStraightCmPID_lib(17, 180, 100, MOVE_BACKWARD, 1.5, 0, 1, 1, 0, 5, 1000, 1, hardwareParameter);

	// armAction_1 = {127, 0, 0, 1};
	// turnDegreesPID_lib(245, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 25, 1100, 2, hardwareParameter);
	// intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};

	// ////take middle high yellow goal
	// delay(100); //
	// goStraightCm_Front_Vision_limit_switch(100, 230, 70, DETECT_YELLOW_GOAL_SIG, front_vision,
	// 							  0.5, 0, 1, 0.3, 0, 10, 0.3, 0, 1, 1500, 1, hardwareParameter);
	// intakeAction_1 = {0, 0, 0, 0, 1};
	// // angle = get_robot_heading_lib(hardwareParameter);
	// //		  goStraightCmPID_lib(32, angle, 50, MOVE_FORWARD, 3.5, 0, 2.5, 0.4, 0, 0, 1000, 1, hardwareParameter);
	// //		  goStraightCmPID_lib_limit_switch(32, angle, 60, MOVE_FORWARD, 3.5, 0, 2.5, 0.4, 0, 0, 1000, 1, hardwareParameter);
	// // delay(150);
	// clawAction_1 = {0, true, 1};
	// delay(claw_action_delay_time);

	// //			armAction_1 = {127, 0, PRESS_BRIDGE + 170, 1};
	// armAction_1 = {127, 0, PRESS_BRIDGE + 80, 1};
	// turnDegreesPID_lib(90, ON_SPOT_TURN, 127, COUNTER_CLOCKWISE, 6, 0, 27, 1600, 2, hardwareParameter);
	// intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	// delay(350);
	// angle = 95;

	// waitForTouch();

	// goStraightCmPID_lib(90, angle, 85, MOVE_FORWARD, 1.5, 0, 0, 1, 0, 5, 1500, 1, hardwareParameter);
	// // move high yellow goal to red bridge
	// armAction_1 = {127, 0, PRESS_BRIDGE, 1};
	// delay(300);
	// goStraightCmPID_lib(15, angle, 80, MOVE_FORWARD, 1.5, 0, 0, 1, 0, 5, 700, 1, hardwareParameter);
	// //			armAction_1 = {127, 0, PRESS_BRIDGE - 20, 1};
	// // armAction_1 = {127, 0, PRESS_BRIDGE - 30, 1};
	// delay(500);
	// //			turnDegreesPID_lib(80, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, -20, 700, 2, hardwareParameter);
	// ////place high yellow mobile goal
	// clawAction_1 = {0, false, 1};
	// // armAction_1 = {127, 0, PRESS_BRIDGE - 40, 1};
	// delay(claw_action_delay_time + 200);
	// goStraightCmPID_lib(5, angle, 127, MOVE_BACKWARD, 3.5, 0, 2.5, 4, 0, 5, 400, 1, hardwareParameter);
	// // armAction_1 = {127, 0, PRESS_BRIDGE + 100, 1};
	// delay(200);

	// waitForTouch();

	// intakeAction_1 = {0, 0, 0, 0, 1};
	// goStraightCmPID_lib(60, 90, 127, MOVE_BACKWARD, 3, 0, 1, 0.5, 0, 2, 1200, 1, hardwareParameter);
	// goStraightCmPID_lib(100, 135, 127, MOVE_BACKWARD, 2, 0, 1, 0.5, 0, 2, 1300, 1, hardwareParameter);
	// goStraightCmPID_lib(110, 145, 127, MOVE_BACKWARD, 2, 0, 1, 0.5, 0, 2, 1400, 1, hardwareParameter);
	// hookAction_1 = {0, false, 1};
	// goStraightCmPID_lib(25, 145, 80, MOVE_BACKWARD, 2, 0, 1, 0.5, 0, 0, 600, 1, hardwareParameter);

	// intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	// goStraightCmPID_lib(68, 130, 127, MOVE_FORWARD, 1.5, 0, 1, 0.5, 0, 7, 1100, 1, hardwareParameter);
	// turnDegreesPID_lib(180, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, -20, 700, 2, hardwareParameter);

	// intakeAction_1 = {0, 0, 0, 0, 1};
	// distance = get_distance_back_vision(back_vision, DETECT_RED_GOAL_SIG, 10, 10, 60, 120);
	// if (distance > 70 || distance < 10)
	// {
	// 	distance = 50;
	// }
	// goStraightCm_Back_Vision(distance + 20, 180, 70, DETECT_RED_GOAL_SIG, back_vision,
	// 						 0.5, 0, 1, 0.5, 0, 5, 0.5, 0, 1, 1000, 1, hardwareParameter);
	// // take corner blue goal
	// hookAction_1 = {0, true, 1};
	// delay(hook_action_delay_time);
	// intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};

	// armAction_1 = {127, 0, 0, 1};
	// goStraightCmPID_lib(50, 180, 90, MOVE_FORWARD, 2, 0, 1, 0.5, 0, 5, 800, 1, hardwareParameter); // 900
	// turnDegreesPID_lib(90, ON_SPOT_TURN, 80, CLOCKWISE, 6, 0, -20, 800, 2, hardwareParameter);	   // 900

	// goStraightCm_Front_Vision(30, 90, 90, DETECT_YELLOW_GOAL_SIG, front_vision,
	// 						  0.5, 0, 1, 0.3, 0, 10, 0.4, 0, 0, 800, 1, hardwareParameter);
	// angle = get_robot_heading_lib(hardwareParameter);
	// //      goStraightCmPID_lib_limit_switch(30, angle, 70, MOVE_FORWARD, 1.5, 0, 1.5, 0.8, 0, 0, 800, 1, hardwareParameter);
	// // take last yellow goal
	// clawAction_1 = {0, true, 1};
	// delay(claw_action_delay_time);
	// armAction_1 = {127, 0, PRESS_BRIDGE + 200, 1};
	// goStraightCmPID_lib(36, 120, 80, MOVE_FORWARD, 3.5, 0, 2.5, 0.4, 0, 3, 800, 1, hardwareParameter);	  // 2000
	// goStraightCmPID_lib(115, 130, 100, MOVE_FORWARD, 3.5, 0, 2.5, 0.4, 0, 3, 1500, 1, hardwareParameter); // 2000

	// armAction_1 = {127, 0, PRESS_BRIDGE, 1};
	// goStraightCmPID_lib(15, 110, 127, MOVE_FORWARD, 3.5, 0, 1, 0.4, 0, 0, 500, 1, hardwareParameter);
	// // drop last yellow goal
	// clawAction_1 = {0, false, 1};
	// delay(claw_action_delay_time);
	// goStraightCmPID_lib(5, 125, 100, MOVE_BACKWARD, 3.5, 0, 2.5, 3, 0, 5, 400, 1, hardwareParameter);
	// armAction_1 = {127, 0, PRESS_BRIDGE + 100, 1};
	// hookAction_1 = {200, false, 1};
	// goStraightCmPID_lib(55, 120, 100, MOVE_BACKWARD, 3, 0, 1, 0.35, 0, 5, 1000, 1, hardwareParameter);

	// // hookAction_1 = {0, false, 1};
	// armAction_1 = {127, 0, 0, 1};
	// intakeAction_1 = {0, 0, 0, 0, 1};
	// // delay(hook_action_delay_time);
	// goStraightCmPID_lib(30, 120, 80, MOVE_FORWARD, 5, 0, 1, 0.5, 0, 10, 800, 2, hardwareParameter);
	// turnDegreesPID_lib(300, ON_SPOT_TURN, 127, COUNTER_CLOCKWISE, 6, 0, -20, 1100, 2, hardwareParameter);
	// // goStraightCmPID_lib_limit_switch(45, 300, 60, MOVE_FORWARD,	3, 0, 1, 0.3, 0, 0, 1300, 1, hardwareParameter);
	// // get last red goal
	// // delay(100);
	// clawAction_1 = {0, true, 1};
	// delay(claw_action_delay_time);
	// armAction_1 = {127, 0, PRESS_BRIDGE + 190, 1};
	// turnDegreesPID_lib(110, ON_SPOT_TURN, 70, COUNTER_CLOCKWISE, 6, 0, -20, 1300, 2, hardwareParameter);
	// goStraightCmPID_lib(77, 110, 100, MOVE_FORWARD, 5, 0, 1, 0.3, 0, 2, 1400, 1, hardwareParameter);

	// // release last blue goal to the bridge
	// // armAction_1 = {127, 0, PRESS_BRIDGE + 100, 1};
	// // delay(100);
	// clawAction_1 = {0, false, 1};
	// delay(claw_action_delay_time);

	// armAction_1 = {127, 500, 0, 1};
	// goStraightCmPID_lib(70, 180, 127, MOVE_BACKWARD, 3.5, 0, 1, 1, 0, 2, 1400, 1, hardwareParameter);
	// turnDegreesPID_lib(120, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, -20, 1000, 2, hardwareParameter);

	// distance = get_distance_front_vision(front_vision, DETECT_BLUE_GOAL_SIG, 10, 10, 60, 120);
	// if (distance > 70 || distance < 10)
	// {
	// 	distance = 50;
	// }
	// new_goStraightCm_Front_Vision(15, 120, 70, DETECT_BLUE_GOAL_SIG, front_vision,
	// 							  0.5, 0, 1, 0.3, 0, 10, 0.4, 0, 1, 1000, 1, hardwareParameter);
	// angle = get_robot_heading_lib(hardwareParameter);
	// armAction_1 = {127, 0, 0, 1};
	// //	goStraightCmPID_lib_limit_switch(distance - 10, angle, 50, MOVE_FORWARD,	1, 0, 1, 0.3, 0, 1, 1200, 1, hardwareParameter);
	// // catch the goal beside the bridge
	// clawAction_1 = {0, true, 1};
	// delay(claw_action_delay_time);
	// armAction_1 = {127, 0, 60, 1};
	// goStraightCmPID_lib(200, 83, 127, MOVE_BACKWARD, 2, 0, 1, 0.3, 0, 5, 2500, 1, hardwareParameter);
	// goStraightCmPID_lib(40, 135, 127, MOVE_FORWARD, 1.5, 0, 1, 0.3, 0, 5, 1000, 1, hardwareParameter);
	// goStraightCmPID_lib(90, 135, 127, MOVE_BACKWARD, 2, 0, 1, 0.3, 0, 5, 1500, 1, hardwareParameter);

	// // turnDegreesPID_lib(30, ON_SPOT_TURN, 80, CLOCKWISE, 6, 0, -20, 800, 2, hardwareParameter);
	// // goStraightCmPID_lib(40, 60, 127, MOVE_FORWARD,	1, 0, 1, 0.3, 0, 1, 900, 1, hardwareParameter);
	// // turnDegreesPID_lib(135, ON_SPOT_TURN, 80, COUNTER_CLOCKWISE, 6, 0, -20, 1000, 2, hardwareParameter);
	// // goStraightCmPID_lib(40, 135, 100, MOVE_BACKWARD,	1, 0, 1, 0.3, 0, 5, 1000, 1, hardwareParameter);
	// delay(100);
	// distance = get_distance_back_vision(back_vision, DETECT_BLUE_GOAL_SIG, 10, 10, 60, 110);
	// armAction_1 = {127, 0, PRESS_BRIDGE + 300, 1};
	// goStraightCm_Back_Vision(distance + 8, 135, 100, DETECT_BLUE_GOAL_SIG, back_vision,
	// 						 0.5, 0, 1, 0.5, 0, 5, 0.5, 0, 5, 1200, 1, hardwareParameter);
	// delay(100);
	// hookAction_1 = {0, true, 1};
	// delay(hook_action_delay_time);

	// // //	bridge code

	// //		goStraightCmPID_lib(40, 195, 127, MOVE_FORWARD, 8, 0, 0, 0.3, 0, 5, 1200, 1, hardwareParameter);
	// goStraightCmPID_lib(5, 135, 127, MOVE_FORWARD, 2, 0, 0, 1, 0, 0, 500, 1, hardwareParameter);
	// turnDegreesPID_lib(215, ON_SPOT_TURN, 60, COUNTER_CLOCKWISE, 6, 0, -20, 1200, 2, hardwareParameter);
	// //		delay(100);
	// goStraightCmPID_lib(45, 215, 127, MOVE_FORWARD, 1.2, 0, 0, 0.5, 0, 5, 1200, 1, hardwareParameter);
	// //		goStraightCmPID_lib(35, 180, 127, MOVE_FORWARD, 1.2, 0, 0, 0.5, 0, 5, 1200, 1, hardwareParameter);
	// //		delay(100);
	// armAction_1 = {127, 200, 20, 1};
	// turnDegreesPID_lib(180, ON_SPOT_TURN, 60, CLOCKWISE, 6, 0, -20, 1200, 2, hardwareParameter);

	// //		armAction_1 = {127, 0, 20, 1};
	// delay(200);
	// left_front_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// left_back_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// left_mid_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// right_front_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// right_back_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	// right_mid_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);

	// goStraightCmPID_lib(70, 180, 127, MOVE_FORWARD, 3, 0, 0, 10, 0, 0, 3000, 1, hardwareParameter);
	// balance_bridge_PID_lib(127, -20, 15, 0, 0, 5000, 1, hardwareParameter);
	// goStraightCmPID_lib(1.5, 180, 127, MOVE_BACKWARD, 0, 0, 0, 3, 0, 0, 800, 1, hardwareParameter);

	// // goStraightCmPID_lib(85, 180, 127, MOVE_FORWARD, 3, 0, 0, 10, 0, 0, 3000, 1, hardwareParameter);
	// // balance_bridge_PID_lib(127, -10, 15, 0, 0, 5000, 1, hardwareParameter);
	// // goStraightCmPID_lib(3.5, 180, 127, MOVE_BACKWARD, 0, 0, 0, 1, 0, 0, 800, 1, hardwareParameter);

	// pros::lcd::print(2, "Time=%d", pros::millis() - start_time);
	// std::cout << "Time=%d" << pros::millis() - start_time << std::endl;
	// waitForTouch();

	////////////////////////////////////////
	////////////////////////////////////////
	////////////////////////////////////////
	//	armAction_1 = {127, 0, 100, 1};
	///////////////////////////////////////?????
	//	goStraightCmPID_lib(15, 30, 100, MOVE_BACKWARD,	5, 0, 1, 0.3, 0, 0, 600, 1, hardwareParameter);

	////////////////////////////////////////
	////////////////////////////////////////
	////////////////////////////////////////

	// bridge code
	//  clawAction_1 = {0, true, 1};
	//  delay(claw_action_delay_time);
	//  armAction_1 = {127, 0, PRESS_BRIDGE + 100, 1};
	//  goStraightCmPID_lib(5.5, 180, 80, MOVE_BACKWARD, 0, 0, 0, 5, 0, 0, 500, 1, hardwareParameter);
	//  hookAction_1 = {0, true, 1};
	//  delay(hook_action_delay_time);
	//  delay(1000);
	//  goStraightCmPID_lib(35, 180, 127, MOVE_FORWARD, 3, 0, 0, 1, 0, 5, 1000, 1, hardwareParameter);
	//  armAction_1 = {127, 0, 20, 1};
	//  delay(1000);
	//  //waitForTouch();
	//  left_front_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	//  left_back_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	//  left_mid_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	//  right_front_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	//  right_back_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	//  right_mid_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	//
	//  goStraightCmPID_lib(85, 180, 127, MOVE_FORWARD, 3, 0, 0, 10, 0, 0, 2000, 1, hardwareParameter);
	//  balance_bridge_PID_lib(127, -15, 15, 0, 0, 5000, 1, hardwareParameter);
	//  goStraightCmPID_lib(3, 180, 127, MOVE_BACKWARD, 0, 0, 0, 1, 0, 0, 800, 1, hardwareParameter);
	//

	pros::lcd::print(2, "Time=%d", pros::millis() - start_time);
	waitForTouch();
}

double get_distance_front_vision(Vision vision_sensor, int goal_color_signature,
								 long samplingNumber, long sensingTimeInterval_Millis,
								 int min_width, long timeoutMillis)
{
	double distance = 0;
	double width1;
	vision_object_s_t closest_goal;
	double sum = 0;
	long i = 0;
	long startTime = pros::millis();

	while (i < samplingNumber)
	{
		if (pros::millis() - startTime >= timeoutMillis)
		{
			break;
		}
		closest_goal = vision_sensor.get_by_sig(0, goal_color_signature);
		width1 = (double)(closest_goal.width);
		if (width1 >= min_width)
		{
			sum = sum + width1;
			i++;
		}
		delay(sensingTimeInterval_Millis);
	}
	if (i == 0)
	{
		return -1;
	}

	width1 = sum / ((double)i);
	distance = 224.17 * powf(2.71828, -0.012 * width1);
	//  distance = -2 * 0.00005 * width1 * width1 * width1 + 0.0125 * width1 * width1
	//	           - 2.7984 * width1 + 242.41;
	return distance;
}

double get_distance_back_vision(Vision vision_sensor, int goal_color_signature,
								long samplingNumber, long sensingTimeInterval_Millis,
								int min_width, long timeoutMillis)
{
	double distance = 0;
	double width1;
	vision_object_s_t closest_goal;
	double sum = 0;
	long i = 0;
	long startTime = pros::millis();

	// 	for(i = 0; i < samplingNumber;){
	// 			closest_goal= vision_sensor.get_by_sig(0, goal_color_signature);
	// 			if(closest_goal.width < 60){
	// 				sum = sum + (double)(closest_goal.width);
	// 				i++;
	// 			}
	// 			pros::lcd::print(0, "Width=%d,  M=%d", closest_goal.width, closest_goal.x_middle_coord);
	// 			delay(sensingTimeInterval_Millis);
	// 		}
	// 	pros::lcd::print(1, "avg=%.1f", sum/((double)i));
	// waitForTouch();

	while (i < samplingNumber)
	{
		if (pros::millis() - startTime >= timeoutMillis)
		{
			break;
		}
		closest_goal = vision_sensor.get_by_sig(0, goal_color_signature);
		width1 = (double)(closest_goal.width);
		if (width1 >= min_width)
		{
			sum = sum + width1;
			i++;
		}
		delay(sensingTimeInterval_Millis);
	}
	if (i == 0)
	{
		return -1;
	}

	width1 = sum / ((double)i);
	//	pros::lcd::print(1, "Width=%.1f", width1);
	//	distance = 0 - (0.00002*width1*width1*width1) + (0.014*width1*width1) - (2.9598*width1) + 242.31;
	distance = 217.84 * powf(2.71828, -0.012 * width1);
	// distance = -1 * 0.0000002 * powf(width1, 3) + 0.0001 * powf(width1, 2) - 0.026 * width1 + 2.4536;

	return distance;
}

int get_frontYellow_width()
{
	vision_object_s_t front_yellow;
	int w = 0;
	int cnt = 10;

	for (int i = 0; i < 10; i++)
	{
		front_yellow = front_vision.get_by_sig(0, DETECT_YELLOW_GOAL_SIG);
		w += front_yellow.width;
		delay(10);
		std::cout << "time " << i + 1 << ": " << front_yellow.width << std::endl;
	}

	w = w / cnt;

	return w;
}

void goStraightCmPID_lib_backVision(int distance, int angle, int speed, int timeout)
{
	vision_object_s_t closest_goal_1;
	vision_object_s_t closest_goal_2;
	double w1 = 0, w2 = 0;
	int cnt = 20;

	for (int i = 0; i < 20; i++)
	{
		closest_goal_1 = back_vision.get_by_sig(0, DETECT_RED_GOAL_SIG);
		w1 += closest_goal_1.width;
		closest_goal_2 = back_vision.get_by_sig(0, DETECT_BLUE_GOAL_SIG);
		w2 += closest_goal_2.width;
		delay(10);
	}

	w1 = w1 / cnt;
	w2 = w2 / cnt;

	if (w1 > w2)
	{
		goStraightCm_Back_Vision(distance, angle, speed, DETECT_RED_GOAL_SIG, back_vision, 0.5, 0, 1, 0.5, 0, 5, 0.5, 0, 5, timeout, 1, hardwareParameter);
		return;
	}

	if (w2 > w1)
	{
		goStraightCm_Back_Vision(distance, angle, speed, DETECT_BLUE_GOAL_SIG, back_vision, 0.5, 0, 1, 0.5, 0, 5, 0.5, 0, 5, timeout, 1, hardwareParameter);
		return;
	}

	goStraightCmPID_lib(distance, angle, speed, MOVE_BACKWARD, 0.9, 0, 0.5, 1, 0, 0, timeout, 1, hardwareParameter);
	return;
}

void test_vision()
{

	long startTime = pros::millis();

	vision_object_s_t first_goal;
	vision_object_s_t second_goal;
	int e_number1 = errno;
	int e_number2 = errno;

	int exposure = 30;
	int flag1 = 0;
	int flag2 = 0;
	std::cout << "exposure = " << exposure << std::endl;
	front_vision.set_exposure(exposure);
	while (true)
	{
		first_goal.width = 0;
		first_goal.height = 0;
		first_goal.left_coord = 0;
		second_goal.width = 0;
		second_goal.height = 0;
		second_goal.left_coord = 0;
		first_goal = front_vision.get_by_sig(0, DETECT_YELLOW_GOAL_SIG);
		e_number1 = errno;
		errno = 0;
		second_goal = front_vision.get_by_sig(1, DETECT_YELLOW_GOAL_SIG);
		e_number2 = errno;
		errno = 0;
		// ENODEV; 19 - The port cannot be configured as a vision sensor
		// EINVAL; 22 - sig_id is outside the range [1-7]
		// EDOM; 33 - size_id is greater than the number of available objects.
		// EAGAIN; 11 - Reading the vision sensor failed for an unknown reason.

		//  if(pros::millis()-startTime > 5000 && flag1 == 0){
		// 		flag1++;
		// 		exposure = exposure + 30;
		// 		front_vision.set_exposure(exposure);
		// 	} else if(pros::millis()-startTime > 10000 && flag2 == 0){
		// 		flag2++;
		// 		exposure = exposure + 30;
		// 		front_vision.set_exposure(exposure);
		// 	}

		std::cout << pros::millis() - startTime << "    " << first_goal.width << "  " << first_goal.height << "  "
				  << first_goal.left_coord << "  " << e_number1;
		std::cout << "            " << second_goal.width << "  " << second_goal.height << "  "
				  << second_goal.left_coord << "  " << e_number2 << "        " << exposure << std::endl;

		// pros::lcd::print(1, "L=%d, W=%d", closest_goal.left_coord, closest_goal.width);
		delay(50);
	}

	// pros::vision_object_s_t closest_goal;
	// while(true){
	//   closest_goal = front_vision.get_by_sig(0, DETECT_YELLOW_GOAL_SIG);
	// 	 pros::lcd::print(0, "Left=%d, Wid=%d, mid=", closest_goal.left_coord, closest_goal.width, closest_goal.x_middle_coord);
	// 	 delay(100);
	// }

	// goStraightCm_Back_Vision(55, 0, 80, DETECT_RED_GOAL_SIG, back_vision,
	// 												 0.5, 0, 1, 0.3, 0, 5, 0.5, 0, 1, 1200, 1, hardwareParameter);

	// goStraightCm_Front_Vision(35, 0, 100, DETECT_YELLOW_GOAL_SIG, front_vision,
	// 										0.5, 0, 0, 0.3, 0, 7, 1, 0, 1, 1000, 1, hardwareParameter);
	// waitForTouch();
	// sys_initial_robot_heading = 180;
	////turnDegreesPID_lib(0, ON_SPOT_TURN, 127, COUNTER_CLOCKWISE, 1.2, 0, -1, 1200, 1, hardwareParameter);
	// waitForTouch();
}

void vision_dataset()
{
	vision_object_s_t closest_goal;
	back_vision.set_exposure(30);
	while (true)
	{
		//		closest_goal = back_vision.get_by_sig(0, DETECT_RED_GOAL_SIG);

		//		std::cout << "width: " << closest_goal.width << std::endl;
		std::cout << "distance1: " << get_distance_back_vision(back_vision, DETECT_RED_GOAL_SIG, 10, 10, 60, 120) << std::endl;
		delay(50);
	}
}

void right_side()
{
	long start_time = pros::millis();
	int intake_speed = 107;
	sys_initial_to_auton_drifting = inertial_sensor.get_rotation();
	sys_initial_robot_heading = 90;

	hookAction_1 = {0, false, 1};
	transmission_piston.set_value(false);
	top_piston.set_value(true);

	goStraightCmPID_lib_limit_switch(102, 90, 127, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 1500, 1, hardwareParameter);
	clawAction_1 = {0, true, 1};
	delay(25);
	robot_staled = true;
	goStraightCmPID_lib(70, 70, 127, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 15000, 1, hardwareParameter);
	robot_staled = false;
	top_piston.set_value(false);
	transmission_piston.set_value(false);
	turnDegreesPID_lib(310, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	clawAction_1 = {0, false, 1};
	goStraightCmPID_lib(20, 310, 127, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 1500, 1, hardwareParameter);
	goStraightCmPID_lib(20, 310, 127, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 1500, 1, hardwareParameter);
	top_piston.set_value(false);
	turnDegreesPID_lib(132, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	goStraightCmPID_lib(20, 132, 127, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 1500, 1, hardwareParameter);
	int f_w = get_frontYellow_width();
	std::cout << "w: " << f_w << std::endl;
	bool got_yellow = false;

	if (f_w >= 70)
	{
		got_yellow = true;
		goStraightCm_Front_Vision(40, 135, 127, DETECT_YELLOW_GOAL_SIG, front_vision, 0.5, 0, 1, 0.3, 0, 10, 0.8, 0, 0, 1500, 1, hardwareParameter);
		angle = get_robot_heading_lib(hardwareParameter);
		goStraightCmPID_lib(30, angle, 90, MOVE_FORWARD, 0.9, 0, 0.5, 1, 0, 0, 1500, 1, hardwareParameter);
		clawAction_1 = {0, true, 1};
		goStraightCmPID_lib(60, 90, 127, MOVE_BACKWARD, 0.9, 0, 0.3, 1, 0, 0, 1500, 1, hardwareParameter);
		turnDegreesPID_lib(180, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
		std::cout << "\n--------------------------------------------\n";
	}
	else
	{
		goStraightCmPID_lib(10, 132, 100, MOVE_BACKWARD, 0.9, 0, 0.3, 1, 0, 0, 1500, 1, hardwareParameter);
		turnDegreesPID_lib(180, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	}

	goStraightCmPID_lib_backVision(60, 180, 100, 1200);
	angle = get_robot_heading_lib(hardwareParameter);
	goStraightCmPID_lib(35, angle, 127, MOVE_BACKWARD, 0.9, 0, 0.5, 1, 0, 0, 1500, 1, hardwareParameter);
	hookAction_1 = {0, true, 1};

	if (got_yellow)
	{
		armAction_1 = {127, 0, 200, 1};
		intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
		goStraightCmPID_lib(30, 180, 50, MOVE_FORWARD, 0.9, 0, 0.5, 1, 0, 0, 1500, 1, hardwareParameter);
	}
	else
	{
		goStraightCmPID_lib(10, 180, 127, MOVE_FORWARD, 0.9, 0, 0.5, 1, 0, 0, 1500, 1, hardwareParameter);
		turnDegreesPID_lib(270, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
		goStraightCm_Front_Vision(25, 270, 127, DETECT_YELLOW_GOAL_SIG, front_vision, 0.5, 0, 1, 0.3, 0, 10, 0.8, 0, 0, 1500, 1, hardwareParameter);
		angle = get_robot_heading_lib(hardwareParameter);
		goStraightCmPID_lib(20, angle, 90, MOVE_FORWARD, 0.9, 0, 0.5, 1, 0, 0, 1500, 1, hardwareParameter);
		clawAction_1 = {0, true, 1};
		intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	}
}

void right_side2()
{
	long start_time = pros::millis();
	int intake_speed = 107;
	sys_initial_to_auton_drifting = inertial_sensor.get_rotation();
	sys_initial_robot_heading = 90;

	clawAction_1 = {0, false, 1};
	hookAction_1 = {0, false, 1};
	transmission_piston.set_value(false);
	top_piston.set_value(true);

	clawAction_1 = {1050, true, 1};
	goStraightCmPID_lib_limit_switch(100, 90, 127, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 1500, 1, hardwareParameter);
	clawAction_1 = {0, true, 1};
	delay(30);
	robot_staled = true;
	goStraightCmPID_lib(50, 70, 127, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 15000, 1, hardwareParameter);
	robot_staled = false;
	transmission_piston.set_value(false);
	turnDegreesPID_lib(180, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	goStraightCmPID_lib_backVision(20, 180, 70, 1200);
	double angle = get_robot_heading_lib(hardwareParameter);
	goStraightCmPID_lib(20, angle, 70, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 1500, 1, hardwareParameter);
	hookAction_1 = {0, true, 1};

	clawAction_1 = {200, false, 1};
	goStraightCmPID_lib(50, 180, 127, MOVE_FORWARD, 1.8, 0, 2.0, 1, 0, 0, 1500, 1, hardwareParameter);
	goStraightCmPID_lib(50, 180, 127, MOVE_BACKWARD, 1.8, 0, 2.0, 1, 0, 0, 1500, 1, hardwareParameter);
	turnDegreesPID_lib(90, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	goStraightCmPID_lib(95, 90, 50, MOVE_FORWARD, 1.8, 0, 2.0, 1, 0, 0, 1500, 1, hardwareParameter);
	intakeAction_1 = {0, 0, 0, 0, 1};
	turnDegreesPID_lib(180, ON_SPOT_TURN, 70, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};

	goStraightCm_Front_Vision(80, 180, 70, DETECT_YELLOW_GOAL_SIG, front_vision, 0.5, 0, 1, 0.3, 0, 10, 0.8, 0, 0, 1500, 1, hardwareParameter);
	angle = get_robot_heading_lib(hardwareParameter);
	goStraightCmPID_lib(40, angle, 70, MOVE_FORWARD, 0.9, 0, 0.5, 1, 0, 0, 1500, 1, hardwareParameter);
	clawAction_1 = {0, true, 1};
	delay(100);
	armAction_1 = {127, 0, 600, 1};
	goStraightCmPID_lib(150, 110, 127, MOVE_BACKWARD, 1.8, 0, 2.0, 1, 0, 0, 1500, 1, hardwareParameter);
}

void right_midG() // 9, 5.5
{
	long start_time = pros::millis();
	int intake_speed = 107;
	sys_initial_to_auton_drifting = inertial_sensor.get_rotation();
	sys_initial_robot_heading = 122;

	clawAction_1 = {0, false, 1};
	hookAction_1 = {0, false, 1};
	transmission_piston.set_value(false);
	top_piston.set_value(true);

	clawAction_1 = {1300, true, 1};
	goStraightCmPID_lib_limit_switch(133, 122, 127, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0.02, 0, 1500, 1, hardwareParameter);
	clawAction_1 = {0, true, 1};
	delay(30);
	robot_staled = true;
	goStraightCmPID_lib(47, 122, 127, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 15000, 1, hardwareParameter);
	robot_staled = false;
	transmission_piston.set_value(false);
	turnDegreesPID_lib(180, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	goStraightCmPID_lib_backVision(50, 180, 100, 1200);
	double angle = get_robot_heading_lib(hardwareParameter);
	goStraightCmPID_lib(30, angle, 70, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 1000, 1, hardwareParameter);
	hookAction_1 = {0, true, 1};

	goStraightCmPID_lib(20, 180, 127, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 500, 1, hardwareParameter);
	clawAction_1 = {0, false, 1};
	armAction_1 = {127, 200, 400, 1};
	goStraightCmPID_lib(40, 180, 127, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 1000, 1, hardwareParameter);
	turnDegreesPID_lib(120, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);

	int f_w = get_frontYellow_width();
	std::cout << "w: " << f_w << std::endl;
	bool got_yellow = false;

	if (f_w >= 75)
	{
		intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
		armAction_1 = {127, 0, 0, 1};
		goStraightCm_Front_Vision(70, 120, 100, DETECT_YELLOW_GOAL_SIG, front_vision, 0.5, 0, 1, 0.3, 0, 10, 0.8, 0, 0, 1500, 1, hardwareParameter);
		double angle = get_robot_heading_lib(hardwareParameter);
		goStraightCmPID_lib(30, angle, 70, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 1000, 1, hardwareParameter);
		clawAction_1 = {0, true, 1};
		armAction_1 = {127, 200, 100, 1};
		goStraightCmPID_lib(100, 90, 127, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 2000, 1, hardwareParameter);
	}
	else
	{
		armAction_1 = {127, 0, 0, 1};
		turnDegreesPID_lib(180, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
		delay(200);
		intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
		goStraightCm_Front_Vision(50, 180, 100, DETECT_YELLOW_GOAL_SIG, front_vision, 0.5, 0, 1, 0.3, 0, 10, 0.8, 0, 0, 1500, 1, hardwareParameter);
		clawAction_1 = {0, true, 1};
	}
}

void left_side()
{
	long start_time = pros::millis();
	int intake_speed = 107;
	sys_initial_to_auton_drifting = inertial_sensor.get_rotation();
	sys_initial_robot_heading = 85;

	hookAction_1 = {0, false, 1};
	transmission_piston.set_value(false);
	top_piston.set_value(true);

	// clawAction_1 = {1050, true, 1};
	goStraightCmPID_lib_limit_switch(108, 85, 127, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 1500, 1, hardwareParameter);
	clawAction_1 = {0, true, 1};
	delay(40);
	robot_staled = true;
	goStraightCmPID_lib(107, 81, 127, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0.05, 15000, 1, hardwareParameter);
	robot_staled = false;
	top_piston.set_value(false);
	transmission_piston.set_value(false);
	turnDegreesPID_lib(172, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	clawAction_1 = {0, false, 1};
	goStraightCmPID_lib_backVision(40, 180, 70, 1200);
	hookAction_1 = {0, true, 1};
	goStraightCmPID_lib(70, 90, 127, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 1500, 1, hardwareParameter);
	turnDegreesPID_lib(0, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	goStraightCmPID_lib(95, 5, 60, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 2000, 1, hardwareParameter);
	turnDegreesPID_lib(90, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1500, 2, hardwareParameter);
	int f_y = get_frontYellow_width();
	std::cout << "front_yellow: " << f_y << std::endl;
	bool mid_y = false;

	if (f_y >= 90)
	{
		mid_y = true;
		delay(100);
		goStraightCm_Front_Vision(40, 90, 100, DETECT_YELLOW_GOAL_SIG, front_vision, 0.5, 0, 1, 0.3, 0, 10, 0.8, 0, 0, 1500, 1, hardwareParameter);
		double angle = get_robot_heading_lib(hardwareParameter);
		goStraightCmPID_lib(20, angle, 127, MOVE_FORWARD, 0.9, 0, 0.5, 1, 0, 0, 1500, 1, hardwareParameter);
		clawAction_1 = {0, true, 1};
		armAction_1 = {127, 200, 100, 1};
		goStraightCmPID_lib(60, 90, 100, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 2000, 1, hardwareParameter);
		turnDegreesPID_lib(0, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	}
	else
	{
		turnDegreesPID_lib(0, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	}

	if (mid_y)
	{
		goStraightCmPID_lib(70, 0, 100, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 1000, 1, hardwareParameter);
	}
	else
	{
		goStraightCmPID_lib(35, 0, 100, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 5000, 1, hardwareParameter);
		turnDegreesPID_lib(180, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 1, hardwareParameter);
		goStraightCm_Front_Vision(50, 90, 127, DETECT_YELLOW_GOAL_SIG, front_vision, 0.5, 0, 1, 0.3, 0, 10, 0.8, 0, 0, 1500, 1, hardwareParameter);
		double angle = get_robot_heading_lib(hardwareParameter);
		goStraightCmPID_lib(35, angle, 127, MOVE_FORWARD, 0.9, 0, 0.5, 1, 0, 0, 1500, 1, hardwareParameter);
		clawAction_1 = {0, true, 1};
	}
}

void right_wp()
{
	long start_time = pros::millis();
	int intake_speed = 107;
	sys_initial_to_auton_drifting = inertial_sensor.get_rotation();
	sys_initial_robot_heading = 90;

	clawAction_1 = {0, false, 1};
	hookAction_1 = {0, false, 1};
	transmission_piston.set_value(false);
	top_piston.set_value(true);

	clawAction_1 = {1050, true, 1};
	goStraightCmPID_lib_limit_switch(100, 90, 127, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 1500, 1, hardwareParameter);
	clawAction_1 = {0, true, 1};
	delay(30);
	robot_staled = true;
	goStraightCmPID_lib(50, 70, 127, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 15000, 1, hardwareParameter);
	robot_staled = false;
	transmission_piston.set_value(false);
	turnDegreesPID_lib(180, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	goStraightCmPID_lib_backVision(40, 180, 70, 1200);
	double angle = get_robot_heading_lib(hardwareParameter);
	goStraightCmPID_lib(30, angle, 70, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 700, 1, hardwareParameter);
	hookAction_1 = {0, true, 1};
	armAction_1 = {127, 0, 400, 1};
	goStraightCmPID_lib(48, 140, 127, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 8000, 1, hardwareParameter);
	intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	goStraightCmPID_lib(160, 192, 60, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 8000, 1, hardwareParameter);

	intakeAction_1 = {0, 0, 0, 0, 1};
	armAction_1 = {127, 0, 600, 1};
	hookAction_1 = {100, false, 1};
	goStraightCmPID_lib(48, 192, 70, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 2000, 1, hardwareParameter);
	turnDegreesPID_lib(90, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	goStraightCmPID_lib(120, 80, 127, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 900, 1, hardwareParameter);
	turnDegreesPID_lib(173, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	goStraightCmPID_lib_backVision(80, 173, 127, 1200);
	hookAction_1 = {0, true, 1};
	delay(40);
	intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	goStraightCmPID_lib(100, 180, 60, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 5000, 1, hardwareParameter);
	std::cout << "time: " << pros::millis() - start_time << std::endl;
}

void right_mid_wp()
{
	long start_time = pros::millis();
	int intake_speed = 107;
	sys_initial_to_auton_drifting = inertial_sensor.get_rotation();
	sys_initial_robot_heading = 122;

	clawAction_1 = {0, false, 1};
	hookAction_1 = {0, false, 1};
	transmission_piston.set_value(false);
	top_piston.set_value(true);

	clawAction_1 = {1300, true, 1};
	goStraightCmPID_lib_limit_switch(133, 122, 127, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0.02, 0, 1500, 1, hardwareParameter);
	clawAction_1 = {0, true, 1};
	delay(50);
	robot_staled = true;
	goStraightCmPID_lib(47, 122, 127, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 15000, 1, hardwareParameter);
	robot_staled = false;
	transmission_piston.set_value(false);
	turnDegreesPID_lib(180, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	goStraightCmPID_lib_backVision(80, 180, 100, 1200);
	hookAction_1 = {0, true, 1};

	armAction_1 = {127, 0, 400, 1};
	goStraightCmPID_lib(48, 140, 127, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 8000, 1, hardwareParameter);
	intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	goStraightCmPID_lib(160, 192, 60, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 8000, 1, hardwareParameter);

	intakeAction_1 = {0, 0, 0, 0, 1};
	armAction_1 = {127, 0, 600, 1};
	hookAction_1 = {100, false, 1};
	goStraightCmPID_lib(47, 192, 70, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 2000, 1, hardwareParameter);
	turnDegreesPID_lib(82, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	goStraightCmPID_lib(120, 80, 127, MOVE_BACKWARD, 1.8, 0, 2.5, 1, 0, 0, 900, 1, hardwareParameter);
	turnDegreesPID_lib(170, ON_SPOT_TURN, 100, COUNTER_CLOCKWISE, 6, 0, 0.1, 1200, 2, hardwareParameter);
	goStraightCmPID_lib_backVision(80, 170, 127, 1200);
	hookAction_1 = {0, true, 1};
	delay(40);
	intakeAction_1 = {intake_speed, 0, 0, intake_speed, 1};
	goStraightCmPID_lib(100, 180, 60, MOVE_FORWARD, 1.8, 0, 2.5, 1, 0, 0, 5000, 1, hardwareParameter);
	std::cout << "time: " << pros::millis() - start_time << std::endl;
}

void left_wp()
{
	long start_time = pros::millis();
	int intake_speed = 107;
	sys_initial_to_auton_drifting = inertial_sensor.get_rotation();
	sys_initial_robot_heading = 90;

	clawAction_1 = {0, false, 1};
	hookAction_1 = {0, false, 1};
	transmission_piston.set_value(false);
	top_piston.set_value(true);
}

void width_test()
{
	int f_y = get_frontYellow_width();
	std::cout << "front_yellow: " << f_y << std::endl;
}

void get_backDistance()
{
	while (true)
	{
		int x = get_distance_back_vision(back_vision, DETECT_BLUE_GOAL_SIG, 10, 10, 20, 1000);
		pros::lcd::print(4, "back blue distance: %d", x);
		waitForTouch();
	}
}

void encoder_test()
{
	while(true)
	{
		std::cout << "encoder: " << Y_encoder.get_value() << std::endl;
	}
}

/**************************
AUTON
**************************/
void autonomous()
{
	arm_motor.set_brake_mode(E_MOTOR_BRAKE_HOLD);
	intake_motor.set_brake_mode(E_MOTOR_BRAKE_BRAKE);

	left_front_motor.set_brake_mode(E_MOTOR_BRAKE_COAST);
	left_back_motor.set_brake_mode(E_MOTOR_BRAKE_COAST);
	left_mid_motor.set_brake_mode(E_MOTOR_BRAKE_COAST);
	right_front_motor.set_brake_mode(E_MOTOR_BRAKE_COAST);
	right_back_motor.set_brake_mode(E_MOTOR_BRAKE_COAST);
	right_mid_motor.set_brake_mode(E_MOTOR_BRAKE_COAST);

	// left_front_motor.set_brake_mode(E_MOTOR_BRAKE_BRAKE);
	// left_back_motor.set_brake_mode(E_MOTOR_BRAKE_BRAKE);
	// left_mid_motor.set_brake_mode(E_MOTOR_BRAKE_BRAKE);
	// right_front_motor.set_brake_mode(E_MOTOR_BRAKE_BRAKE);
	// right_back_motor.set_brake_mode(E_MOTOR_BRAKE_BRAKE);
	// right_mid_motor.set_brake_mode(E_MOTOR_BRAKE_BRAKE);

	left_back_motor.set_zero_position(0.0);
	left_front_motor.set_zero_position(0.0);
	left_mid_motor.set_zero_position(0.0);
	right_back_motor.set_zero_position(0.0);
	right_front_motor.set_zero_position(0.0);
	right_mid_motor.set_zero_position(0.0);
	intake_motor.set_zero_position(0.0);
	arm_motor.set_zero_position(0.0);

	sys_initial_to_auton_drifting = hardwareParameter.inertialSensorLib.get_rotation();

	//   //this line records the inertial sensor drifting between initialization and start of auton.
	//  	sys_initial_robot_heading = 270;
	// 	turnDegreesPID_Parameter = {0, ON_SPOT_TURN, 100, CLOCKWISE, 6, 0, -20, 1500, 2};
	//   pros::Task background(background_execution_turnDegreesPID_lib, (void*)"PROS", TASK_PRIORITY_DEFAULT, TASK_STACK_DEPTH_DEFAULT, "turn");

	//   double distance = get_distance_back_vision(back_vision, DETECT_BLUE_GOAL_SIG, 50, 100, 60, 10000);

	// waitForTouch();

	// waitForTouch();
	// //	pros::lcd::print(2, "Time=%d", pros::millis() - start_time);

	/***********************************************

	CHOOSE RUN HERE

	***********************************************/

	front_piston.set_value(false);

	right_side();
	// right_side2();
	// right_midG();
	// right_wp();
	// right_mid_wp();
	// left_side();
	// left_wp();
	// auton_60s_skills_bridge_version();
	// get_backDistance();

	// encoder_test();

	waitForTouch();
}
